...
class MyBot(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        self.autodj = config.get("autodj", False)
        ...
        self.default_english_songs = [
            ...
            "Closer - The Chainsmokers", "Love Yourself - Justin Bieber", "7 Rings - Ariana Grande","Blinding Lights - The Weeknd", "Shape of You - Ed Sheeran", "Stay - Justin Bieber", "Believer - Imagine Dragons", "Starboy - The Weeknd", "Flowers - Miley Cyrus", "Perfect - Ed Sheeran", "Uptown Funk - Bruno Mars", "Closer - The Chainsmokers", "Levitating - Dua Lipa", "Bad Guy - Billie Eilish", "As It Was - Harry Styles", "Heat Waves - Glass Animals", "Someone You Loved - Lewis Capaldi", "Senorita - Shawn Mendes", "Cruel Summer - Taylor Swift", "Sunflower - Post Malone", "Havana - Camila Cabello", "Despacito - Luis Fonsi", "Old Town Road - Lil Nas X", "Cheap Thrills - Sia", "7 Rings - Ariana Grande", "Lovely - Billie Eilish", "Counting Stars - OneRepublic", "Wake Me Up - Avicii", "Let Me Love You - DJ Snake", "Unstoppable - Sia", "Rockstar - Post Malone", "Cold Heart - Elton John", "Watermelon Sugar - Harry Styles", "Peaches - Justin Bieber", "Don't Start Now - Dua Lipa", "Thinking Out Loud - Ed Sheeran", "One Kiss - Calvin Harris", "Save Your Tears - The Weeknd", "Memories - Maroon 5", "Girls Like You - Maroon 5", "Stressed Out - Twenty One Pilots", "Treat You Better - Shawn Mendes", "New Rules - Dua Lipa", "Dusk Till Dawn - Zayn", "Thrift Shop - Macklemore", "Can't Stop The Feeling - Justin Timberlake", "Hello - Adele", "Rolling in the Deep - Adele", "Take Me To Church - Hozier", "All of Me - John Legend", "Sugar - Maroon 5", "Radioactive - Imagine Dragons", "Just The Way You Are - Bruno Mars"
        ]
    ...
    async def play_song_to_icecast(self, song_info):
        ...
        if not yt_link:
            print("No link found for song, skipping stream.")
            return False
        ...
        if not direct_url:
            print(f"Could not get direct audio URL for {yt_link}")
            return False
        ...
        try:
            ...
            if not icecast_url:
                print("No icecast_url found in config.json")
                return False

            # ffmpeg -re -i [INPUT] -acodec libmp3lame -ab 128k -f mp3 [URL]
            title = song_info.get("song", "Unknown Song")
            # Sanitize title for metadata to avoid issues with special characters on Windows
            safe_title = "".join([c if ord(c) < 128 else " " for c in title]).replace('"', "'")
            
            # Improved ffmpeg command with reconnection flags for stability
            command = [
                "ffmpeg",
                "-reconnect", "1",
            ...
            print(f"Starting stream: {' '.join(command)}")
            ...
            print(f"Failed to start stream process: {e}")
            return False
    ...
    async def auto_dj_loop(self):
        ...
        while True:
            ...
            if not getattr(self, 'bot_enabled', True):
                continue
            try:
                ...
                # 2. If no stream is running and we have songs in the queue, start the next one
                if self.stream_process is None and len(self.song_queue) > 0:
                    ...
                    if success:
                        await asyncio.sleep(2)
                        try:
                            with open(get_path("config.json")) as _f:
                                _cfg = json.load(_f)
                        except Exception:
                            _cfg = {}
                        if _cfg.get("paid_mode", False):
                            requester_id = current_song.get("user_id")
                            if requester_id:
                                np_msg = self.get_room_np_message()
                                await self.highrise.send_whisper(requester_id, np_msg)
                    else:
                        # Skip this song if it couldn't be played
                        print(f"Auto DJ: Skipping '{song_name}' due to playback failure.")
                        await self.highrise.chat(f"⚠️ Error: Could not play '{song_name}'. Skipping to next song...")
                        if len(self.song_queue) > 0:
                            self.song_queue.pop(0)


                # 3. Handle AutoDJ English Song Playback
                if self.autodj and len(self.song_queue) == 0:
                    ...
                    if yt_title:
                        ...
                        print(f"AutoDJ: Could not find '{random_song}'. Waiting a bit...")
    ...
    async def send_room_chunks(self, user_id, message_text, chunk_size=250):
        """Send a whisper message, splitting into chunks if necessary (safely)."""
        ...
        for line in lines:
            ...
            if len(line) > chunk_size:
                if current_chunk:
                    await self.highrise.send_whisper(user_id, current_chunk.strip())
                    await asyncio.sleep(0.5)
                    current_chunk = ""
                
                # Split long line into parts
                for i in range(0, len(line), chunk_size):
                    await self.highrise.send_whisper(user_id, line[i:i+chunk_size])
                    await asyncio.sleep(0.5)
                continue
    ...
    def get_room_np_message(self) -> str:
        ...
        return (f"<#FFD700>🎵 𝐍𝐨𝐰 𝐏𝐥𝐚𝐲𝐢𝐧𝐠:\n"
                f"<#cdaa54>{title}\n"
                f"<#e2ce9d> ▷ •ıı|ıı||ı• ({mins_total}:{secs_total:02d})\n"
                f"<#e2ce9d> 👤 @{requester} | ❤️ {likes} | 🎧 {listens}")
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        ...
        await self.highrise.send_whisper(user.id, f"🎉 @{user.username} just joined the party! 🎉 Type -sub to unlock exclusive perks and enjoy the music 🎵!")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        try:
            ...
            if msg == "-bot off":
                ...
                await self.highrise.send_whisper(user.id, "🔴 Bot music disabled.")
                return
            ...
            if msg == "-bot on":
                if user.username not in owners:
                    return
                self.bot_enabled = True
                await self.highrise.send_whisper(user.id, "🟢 Bot music enabled.")
                return

            # Silently ignore all music commands when bot is disabled (non-owners only)
            _MUSIC_CMDS = (
                "-play", "-skip", "-q", "-queue", "-np", "-like", "-dislike",
                "-ql", "-fav", "-myslots", "-buyslots", "-give", "-paidmode",
                "-autodj", "-vipcost", "-favplay", "-favlist", "-favadd", "-favdel",
            )
            if not getattr(self, 'bot_enabled', True) and user.username not in owners:
                if any(msg == c or msg.startswith(c + " ") for c in _MUSIC_CMDS):
                    return

            if msg == "-ping":
                await self.highrise.send_whisper(user.id, f"Pong! {user.username}")
                ...
                try:
                    ...
                    if user.username not in subs:
                        ...
                        await self.highrise.send_whisper(user.id, f"✅ @{user.username}, you are now SUBSCRIBED! All music commands are UNLOCKED for you. 🎉")
                ...
                try:
                    if os.path.exists(get_path("subscribed_users.json")):
                        ...
                        if user.username in subs:
                            ...
                            with open(get_path("subscribed_users.json"), "w") as f:
                                json.dump(subs, f, indent=4)
                            await self.highrise.send_whisper(user.id, f"❌ @{user.username}, you have UNSUBSCRIBED. Music commands are now LOCKED. Type -sub to unlock again.")
                        else:
                ...
                await self.highrise.send_whisper(user.id, play_msg)
                ...
                await self.highrise.send_whisper(user.id, status_msg)
                ...
                if getattr(self, "song_queue", None) and len(self.song_queue) > 0:
                    ...
                    if hasattr(self, "get_room_np_message"):
                        ...
                        await self.highrise.send_whisper(user.id, msg_np)
                else:
                ...
                if is_staff or is_requester:
                    ...
                    if len(self.song_queue) > 0:
                        ...
                        await self.highrise.send_whisper(user.id, f"⏭️ @{user.username} skipped: {skipped_song.get('song', 'the song')}")
                    # Reset song_stats so -np doesn't show stale data for the skipped song
                    self.song_stats = None

                    # Stop the current stream
                    # The auto_dj_loop will then automatically start the next song
                    await self.stop_stream()
                    
                    # If queue is now empty, announce it
                    if len(self.song_queue) == 0:
                        await self.highrise.send_whisper(user.id, "🎵 Queue is now empty! Use -play [song name] to request the next song. 🎶")
                else:
                ...
                if len(parts) < 3 or not parts[1].startswith("@"):
                    ...
                    gift_msg = (f"<#B8860B>✨ 🎁 𝐒𝐎𝐍𝐆 𝐃𝐄𝐃𝐈𝐂𝐀𝐓𝐈𝐎𝐍 🎁 ✨\n"
                                ...
                                f"<#e2ce9d>🔢 {pos_text}")
                    await self.highrise.send_whisper(user.id, gift_msg)
                ...
                await self.highrise.send_whisper(user.id, f"🎁 @{user.username} gave {amount} music slots to @{target_username}!")
                ...
                await self.highrise.send_whisper(user.id, f"💎 {user.username} successfully bought {vip_name} VIP for {cost} gold!")
                ...
                if target not in config.get("moderators", []):
                    ...
                    await self.highrise.send_whisper(user.id, f"🛡️ @{target} has been made a Moderator by @{user.username}!")
                else:
                ...
                if target in config.get("moderators", []):
                    ...
                    with open(get_path("config.json"), "w") as f:
                        json.dump(config, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"❌ @{target} is no longer a Moderator.")
                else:
                ...
                if target not in config.get("owners", []):
                    ...
                    with open(get_path("config.json"), "w") as f:
                        json.dump(config, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"👑 @{target} has been made an Owner by @{user.username}!")
                else:
                ...
                if target in config.get("owners", []):
                    ...
                    with open(get_path("config.json"), "w") as f:
                        json.dump(config, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"❌ @{target} is no longer an Owner.")
                else:
                ...
                if target not in vip_users:
                    ...
                    with open(get_path("vip_users.json"), "w") as f:
                        json.dump(vip_users, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"💎 @{target} has been given VIP access by @{user.username}!")
                else:
                ...
                if target in vip_users:
                    ...
                    with open(get_path("vip_users.json"), "w") as f:
                        json.dump(vip_users, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"❌ @{target} is no longer a VIP.")
                else:
                ...
                if target not in blocked:
                    ...
                    with open(get_path("config.json"), "w") as f:
                        json.dump(config, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"🚫 @{target} has been BLOCKED. They can no longer use any commands.")
                else:
                ...
                if target in blocked:
                    ...
                    with open(get_path("config.json"), "w") as f:
                        json.dump(config, f, indent=4)
                    await self.highrise.send_whisper(user.id, f"✅ @{target} has been UNBLOCKED.")
                else:
                ...
                for m in unique_mods:
                    role_msg += f"<#e2ce9d>  - @{m}\n"
                await self.highrise.send_whisper(user.id, role_msg)
                ...
                try:
                    ...
                    vip_users = {}
                if user.username not in owners and user.username not in vip_users:
                    await self.highrise.send_whisper(user.id, "You need VIP or Owner role to view the VIP list.")
                    return
                ...
                await self.send_room_chunks(user.id, vip_msg)
                ...
                await self.highrise.send_whisper(user.id, f"✅ VIP {v_type.capitalize()} price updated to {amount} gold!")
                ...
                await self.highrise.send_whisper(user.id, "Sending out invites to everyone... This might take a moment! 📡")
                ...
                try:
                    ...
                    if hasattr(response, "conversations"):
                        ...
                        await self.highrise.send_whisper(user.id, "Failed to load conversations list from the server.")
                        return
                    ...
                    await self.highrise.send_whisper(user.id, f"✅ Successfully sent {count} room invites!")
                except Exception as e:
                    print(f"Exception in get_conversations: {e}")
                    await self.highrise.send_whisper(user.id, f"Error executing invite command: {e}")
                ...
                await self.highrise.send_whisper(user.id, f"🎰 @{user.username} bought {amount} music slots! Enjoy the music! 🎵")
                ...
                if mode == "on":
                    ...
                    await self.highrise.send_whisper(user.id, "📢 Paid Mode is now ON. Slots are required to play music (Except VIP/Staff)!")
                elif mode == "off":
                    config["paid_mode"] = False
                    await self.highrise.send_whisper(user.id, "📢 Paid Mode is now OFF. Everyone can play music for free!")
                else:
                ...
                if mode == "on":
                    self.autodj = True
                    config["autodj"] = True
                    await self.highrise.send_whisper(user.id, "📢 English AutoDJ is now ON. Bot will play songs automatically when queue is empty!")
                elif mode == "off":
                    self.autodj = False
                    config["autodj"] = False
                    await self.highrise.send_whisper(user.id, "📢 English AutoDJ is now OFF.")
                else:
                    await self.highrise.send_whisper(user.id, "Usage: -autodj on/off")
            ...
            # (It was at line 1191 in my previous view)
                ...
                if user.username not in owners:
                    await self.highrise.send_whisper(user.id, "You do not have permission to use that command.")
                    return
                ...
                if not isinstance(room_users, list):
                    ...
                    await self.highrise.send_whisper(user.id, "An error occurred while finding your location.")
                    return
                ...
                for room_user, pos in room_users:
                    if room_user.id == user.id:
                        if isinstance(pos, Position):
                            ...
                            await self.highrise.walk_to(pos)
                            await self.highrise.send_whisper(user.id, "My position has been updated!")
                        else:
                            await self.highrise.send_whisper(user.id, "I cannot move to an AnchorPosition.")
                        break
                ...
                try:
                    ...
                    if not target_name:
                        await self.highrise.send_whisper(user.id, "Usage: -getfit @username")
                        return
                        ...
                        try:
                            async with aiohttp.ClientSession() as session:
                                ...
                                if uid:
                                    ...
                                    if not temp_outfit or len(temp_outfit) == 0:
                                        await self.highrise.send_whisper(user.id, f"⏳ Global search dry... Trying direct Cloud Link for @{target_name} ☁️")
                    ...
                    if target_outfit is None:
                        await self.highrise.send_whisper(user.id, f"❌ Extraction failed for @{target_name}. This error usually means the user is private or their profile data is restricted.")
                        return
                    ...
                    if skipped > 0:
                        await self.highrise.send_whisper(user.id, f"✅ Look applied! (Extraction: {len(final_outfit)} items matched, {skipped} items skipped due to ownership).")
                    else:
                        await self.highrise.send_whisper(user.id, f"🏁 ✨ 𝐏𝐄𝐑𝐅𝐄𝐂𝐓 𝐄𝐗𝐓�𝐀𝐂𝐓𝐈��! I'm now a 1:1 copy of @{target_name}! ✨")
                except Exception as e:
                ...
                try:
                    ...
                    await self.highrise.send_whisper(user.id, f"✅ Outfit '{name}' saved to Slot {slot}! Use -loadfit {slot} to wear it.")
                except Exception as e:
                ...
                try:
                    ...
                    await self.highrise.send_whisper(user.id, f"✨ '{name}' Loaded successfully! ✨")
                except Exception as e:
                ...
                try:
                    if not os.path.exists("outfit_slots.json"):
                        await self.highrise.send_whisper(user.id, "No saved outfit slots yet. Use -savefit [slot] [name] to start!")
                        return
                    with open("outfit_slots.json", "r") as f:
                        slots = json.load(f)
                    
                    if not slots:
                        await self.highrise.send_whisper(user.id, "Your outfit library is currently empty. Use -savefit [slot] [name] to save your first look! ✨")
                        return
                    ...
                    await self.send_room_chunks(user.id, slots_text)
                except Exception as e:
                ...
                try:
                    ...
                    if removed:
                        with open("outfit_slots.json", "w") as f:
                            json.dump(slots, f, indent=4)
                        await self.highrise.send_whisper(user.id, f"🗑️ Outfit '{target}' has been removed successfully!")
                    else:
                ...
                with open(get_path("config.json"), "w") as f:
                    json.dump(config, f, indent=4)
                await self.highrise.send_whisper(user.id, "🔄 Bot outfit has been reset! Please restart the bot to apply standard look.")
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            # Use subprocess to safely handle the execution
